...
const localBancoDados = currentPath + '\\bancoDados.txt'
...
function lerDados(){
    ...
    try {
        ...
        console.log("XXXXXXXX"); // o try pula para o catch logo no primeiro erro
        //leitura das linhas do arquivo
        for(let linha = 0; linha < input.length; linha++){
            let registro = input[linha].split(";")
            cadastroAgenda[linha] = registro
        }
    } catch (erro) { // erro como parametro
        console.clear(erro)
        console.log(erro.message) // dá para acessar detalhes do erro
        console.log("Não foi encontrado o arquivo " + localBancoDados + "!")
        console.log("A aplicação será executada com a memória em branco.")
